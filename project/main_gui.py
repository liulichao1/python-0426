...
class BigWork(Frame):
    ...
    def initUI(self):
        ...
        self.labelImage = Label(self.master, width=125, height=25, bg='white')
        self.labelImage.grid(row=3)
    ...
    def makeImageEven(seif, image):
        ...
        evenPixels = [(r >> 1 << 1, g >> 1 << 1, b >> 1 << 1, t >> 1 << 1) for [r, g, b, t] in
                      pixels]  # 更改所有值为偶数（魔法般的移位）
    ...
    def encodeDataInImage(self, image, data):
        ...
        encodedPixels = [
            (r + int(binary[index * 4 + 0]), g + int(binary[index * 4 + 1]), b + int(binary[index * 4 + 2]),
